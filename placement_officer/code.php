...
if(isset($_POST['pl_officer_login']))
{   
    $email=$_POST['email'];
    $password=md5($_POST['password']);
    $ref="placement_officer/";
        ...
        foreach($login_data as $key => $row)
            {
                if( $row['email'] == $email)
                {
                    ...
                    {
                        $_SESSION['pl_officer_key']=$key;
                        $_SESSION['pl_officer_email']=$email;
                        $_SESSION['pl_officer_name']=$row["name"];
...
if(isset($_POST['pl_officer_logout']))
{

	unset($_SESSION['pl_officer_key']);
    unset($_SESSION['pl_officer_email']);
    unset($_SESSION['pl_officer_name']);
	header('Location: index.php');
}
